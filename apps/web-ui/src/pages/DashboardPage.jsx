...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        s.resume = resumes.length > 0
        if (s.resume) {
          // Check any resume has confirmed roles
          for (const r of resumes) {
            const roles = await api.get(`/resume/roles/${r.resume_id}`)
            if (roles.roles?.some(ro => ro.is_confirmed)) { s.roles = true; break }
          }
        }
      } catch {}
      try {
  ...
  const statCards = [
    { label: 'Resume', value: status.resume ? '✓' : '—', sub: status.resume ? 'Uploaded' : 'Not uploaded', icon: FileText, color: status.resume ? 'text-emerald-400' : 'text-slate-500' },
    { label: 'Jobs', value: recentScores.length || '—', sub: 'recently scored', icon: Briefcase, color: 'text-brand' },
    { label: 'To Apply', value: recentScores.filter(j => j.verdict === 'ASSISTED_APPLY').length || '—', sub: 'APPLY verdict', icon: Send, color: 'text-emerald-400' },
    { label: 'Avg Score', value: recentScores.length ? Math.round(recentScores.reduce((a, j) => a + j.score, 0) / recentScores.length) : '—', sub: 'across scored', icon: Star, color: 'text-amber-400' },
  ]
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statCards.map(({ label, value, sub, icon: Icon, color }) => (
          <div key={label} className="card">
            ...
            <div className={`text-3xl font-bold mb-1 ${color}`}>{status.loading ? <Spinner size="sm" /> : value}</div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <div className="divide-y divide-surface-border">
            ...
            <CheckItem done={status.roles}  label="Confirm target roles" to={status.resume ? undefined : '/resume'} />
            ...
            <CheckItem done={recentScores.some(j => j.verdict === 'ASSISTED_APPLY')} label="Generate artifacts & apply" to="/jobs" />
          </div>
        </div>
        ...
        <div className="card">
          ...
          {status.loading
            ...
            : recentScores.length === 0
              ? <p className="text-sm text-slate-500 py-4">No scored jobs yet. <Link to="/jobs" className="text-brand hover:underline">Go to Jobs →</Link></p>
              : <div className="space-y-2">
                  {recentScores.map(j => (
                    <Link key={j.job_id} to={`/jobs/${j.job_id}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-hover transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-200 truncate">{j.parsed_role || j.title || 'Unknown Role'}</p>
