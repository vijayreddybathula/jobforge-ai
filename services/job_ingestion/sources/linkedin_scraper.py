...
class LinkedInScraper:
    ...
    def _check_rate_limit(self) -> bool:
        ...
        is_allowed, count, remaining = self.rate_limiter.check_rate_limit(
            identifier="linkedin",
            max_requests=self.max_jobs_per_hour,
            window=3600  # 1 hour
        )
        
        if not is_allowed:
            logger.warning(f"Rate limit exceeded for LinkedIn. Current: {count}, Remaining: {remaining}")
            return False
        
        logger.debug(f"LinkedIn rate limit: {count}/{self.max_jobs_per_hour}, Remaining: {remaining}")
        return True
    ...
    def _extract_company(self, page: Page) -> str:
        ...
        try:
            ...
            company = page.locator(".topcard__org-name-link, .job-details-jobs-unified-top-card__company-name").first.inner_text()
    ...
    def _extract_location(self, page: Page) -> str:
        ...
        try:
            ...
            location = page.locator(".topcard__flavor--bullet, .job-details-jobs-unified-top-card__bullet").first.inner_text()
    ...
    def _extract_description(self, page: Page) -> str:
        ...
        try:
            ...
            description = page.locator(".description__text, .jobs-description__text").first.inner_text()
