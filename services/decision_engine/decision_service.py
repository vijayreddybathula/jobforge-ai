...
class DecisionService:
    ...
    def make_decision(
        self, job_id: UUID, user_id: UUID, score: int, verdict: str, db: Optional[Session] = None
    ) -> Dict[str, Any]:
        ...
        if not job:
            return {"decision": "SKIP", "reason": "Job not found"}

        company = job.company or "unknown"

        # Check company cooldown
        if self._check_cooldown(user_id, company):
            return {"decision": "SKIP", "reason": f"Cooldown period active for {company}"}
    ...
    def _increment_application_count(self, user_id: UUID, company: str) -> None:
        ...
        days_until_month_end = (datetime.utcnow().replace(day=28) + timedelta(days=4)).replace(
            day=1
        ) - datetime.utcnow()
