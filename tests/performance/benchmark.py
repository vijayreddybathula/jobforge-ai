...
class PerformanceBenchmark:
    ...
    def test_job_parsing(self, num_jobs: int = 5) -> None:
        ...
        for i in range(num_jobs):
            payload = {"job_description": sample_job_desc, "source": "linkedin"}

            start = time.time()
            try:
                response = requests.post(
                    f"{self.BASE_URL}/api/jobs/parse", json=payload, timeout=30
                )
                elapsed = (time.time() - start) * 1000
                times.append(elapsed)
    ...
    def test_resume_analysis(self, num_resumes: int = 5) -> None:
        ...
        for i in range(num_resumes):
            payload = {"resume_text": sample_resume, "format": "text"}

            start = time.time()
            try:
                response = requests.post(
                    f"{self.BASE_URL}/api/resume/analyze", json=payload, timeout=30
                )
                elapsed = (time.time() - start) * 1000
                times.append(elapsed)
    ...
    def test_job_scoring(self, num_scores: int = 5) -> None:
        ...
        for i in range(num_scores):
            payload = {"job_id": f"job_{i}", "candidate_id": f"candidate_{i}"}

            start = time.time()
            try:
                response = requests.post(
                    f"{self.BASE_URL}/api/jobs/score", json=payload, timeout=30
                )
                elapsed = (time.time() - start) * 1000
                times.append(elapsed)
