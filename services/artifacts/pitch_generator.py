...
class PitchGenerator:
    ...
    def generate(self, parsed_jd: ParsedJD, user_skills: Dict[str, Any], score: int) -> str:
        """Generate recruiter pitch.
        ...
        for skill in parsed_jd.must_have_skills[:5]:
            if any(
                us.lower() in skill.lower() or skill.lower() in us.lower()
        ...
        if matched_skills:
            pitch_parts.append(
                f"I have strong experience with {', '.join(matched_skills[:3])}, which align perfectly with your requirements."
            )
