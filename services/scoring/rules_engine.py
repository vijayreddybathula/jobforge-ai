...
class RulesEngine:
    ...
    def check_constraints(
        ...
        if parsed_jd.red_flags:
            for flag in parsed_jd.red_flags:
                if "citizenship" in flag.lower() or "security clearance" in flag.lower():
                    if user_preferences.visa_status and "citizen" not in user_preferences.visa_status.lower():
                        return False, f"Job requires {flag} but user visa status is {user_preferences.visa_status}"
        ...
        if user_preferences.salary_min_usd and parsed_jd.salary_range:
            if parsed_jd.salary_range.max and parsed_jd.salary_range.max < user_preferences.salary_min_usd:
                return False, f"Job max salary ${parsed_jd.salary_range.max} is below user minimum ${user_preferences.salary_min_usd}"
