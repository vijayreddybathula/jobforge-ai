...
import { Search, Zap, Briefcase, ChevronRight, AlertCircle } from 'lucide-react'
      ...
      <div className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-md p-6 space-y-4">
        ...
        {result && (
          <div className={`text-sm px-3 py-2 rounded-lg border ${
            ...
            {result.error
              ? <span className="flex items-center gap-2"><AlertCircle size={14} />{result.error}</span>
...
export default function JobsPage() {
  ...
  const handleScoreAll = async () => {
    ...
    try {
      ...
      setToast({ message: `Scored ${r.scored} new job${r.scored !== 1 ? 's' : ''}.` })
      await loadJobs()
    } catch (e) {
      setToast({ message: e.message, type: 'error' })
    } finally {
  ...
  const filtered = jobs.filter(j => {
    ...
    if (q && !`${j.title ?? ''} ${j.company ?? ''} ${j.parsed_role ?? ''}`.toLowerCase().includes(q)) return false
    return true
  })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{total} job{total !== 1 ? 's' : ''} in catalog</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card flex items-center gap-3 text-red-400 border-red-500/30">
          ...
          <button onClick={loadJobs} className="ml-auto btn-secondary btn-sm">Retry</button>
        </div>
      ) : filtered.length === 0 ? (
        <EmptyState
          icon={Briefcase}
          ...
          description={
            jobs.length === 0
              ? 'Click "Search Jobs" to pull in jobs from JSearch.'
          ...
          {filtered.map(job => (
            <div key={job.job_id} className="card hover:border-slate-600 transition-colors">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-1.5 mt-1.5 flex-wrap">
                    <span className="text-xs text-slate-500 bg-surface px-2 py-0.5 rounded border border-surface-border">
                      {job.source}
                    </span>
                    <span className="text-xs text-slate-500 bg-surface px-2 py-0.5 rounded border border-surface-border">
                      {job.parse_status}
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-surface-border flex-wrap">
                {!job.score ? (
                  <button
                    onClick={() => handleScoreOne(job.job_id)}
                ...
                {(job.verdict === 'ASSISTED_APPLY' || job.verdict === 'ELIGIBLE_AUTO_SUBMIT') && (
                  <button
                    onClick={() => navigate(`/jobs/${job.job_id}/artifacts`)}
