...
class FeedbackAnalyzer:
    ...
    def analyze_feedback(self, user_id: str, db: Optional[Session] = None) -> Dict[str, Any]:
        ...
        for app in applications:
            score = (
                db.query(JobScore)
                .filter(JobScore.job_id == app.job_id, JobScore.user_id == app.user_id)
                .first()
            )
            ...
            # Get outcomes
            outcomes = db.query(Outcome).filter(Outcome.application_id == app.application_id).all()
        ...
        for band, data in score_bands.items():
            if data["total"] > 0:
                callback_rate = (data["phone_screen"] + data["onsite"] + data["offer"]) / data[
                    "total"
                ]
                analysis[band] = {**data, "callback_rate": round(callback_rate * 100, 2)}
            else:
                analysis[band] = data
